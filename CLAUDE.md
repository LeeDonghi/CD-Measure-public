...
5. **작업 환경/자료는 항상 두 컴퓨터에서 동일하게 쓸 수 있도록 한다.**
   ...
   **다른 컴퓨터에서 이어받을 때 첫 명령은 `git fetch`(또는 `git pull`).**
   `git status`의 "깨끗함"과 `git branch -vv`의 "동기화됨"은 **마지막으로
   fetch한 시점 기준**이라, 다른 PC에서 올린 커밋이 있어도 아무 표시가 없다
   (2026-08-23에 실제로 사흘치를 못 본 채 문서를 쓸 뻔했음). 세션 시작 때
   `SessionStart` 훅(`scripts/session-start-fetch.ps1`)이 자동으로 fetch하고
   뒤처졌으면 경고하니, **그 경고가 뜨면 다른 판단을 하기 전에 `git pull`부터.**
